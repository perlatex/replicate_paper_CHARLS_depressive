...
#DA051
health <- haven::read_dta("Health_Status_and_Functioning2013.dta")  
...
demo$ID %>% typeof()
behavior$ID <- behavior$ID %>% as.numeric()
behavior<-left_join(demo, behavior, by="ID")
names(behavior)

#delete cases if they have all missing values for certain variables 
# 水平方向上，这三个变量不能都为NA
behavior1<-behavior[rowSums(is.na(behavior[,c("vig_total","mod_total","walk_total")]))<length(behavior[,c("vig_total","mod_total","walk_total")]),]
propmiss(behavior)

length(behavior[,c("vig_total","mod_total","walk_total")])
glimpse(behavior)
...
readr::write_rds(mydata, "mydata_author.rds")
readr::write_rds(mydataImp1, "mydataImp1_author.rds")
